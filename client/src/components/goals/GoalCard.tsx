      ...
      <div className="p-6 h-full grid grid-rows-[auto_auto_auto_1fr_auto_auto] gap-4">
        ...
        <div className="h-12 flex items-start justify-between">
          <div className="flex-1">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white line-clamp-2 leading-6">
              {goal.title}
            </h3>
          </div>
        ...
        <div className="h-8 flex items-center justify-between">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <Badge variant={statusInfo.variant} className={`${statusInfo.className} text-xs font-medium px-2.5 py-1 whitespace-nowrap`}>
              <StatusIcon className="w-3 h-3 mr-1" />
              {statusInfo.label}
            </Badge>
            {goal.category && (
              <Badge variant="outline" className="text-xs bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 px-2.5 py-1 whitespace-nowrap">
                {goal.category}
              </Badge>
            )}
          </div>
          {/* On Track Status - Fixed positioning */}
          <div className="flex-shrink-0">
            {goal.status === 'in_progress' && !isOverdue && daysRemaining > 7 ? (
              <div className="inline-flex items-center px-2 py-1 bg-green-50 dark:bg-green-950 rounded-md border border-green-200 dark:border-green-800">
                ...
                <span className="text-xs font-semibold text-green-700 dark:text-green-300 whitespace-nowrap">On track</span>
              </div>
            ) : (
        ...
        {/* Grid Row 3: Description - Fixed height */}
        <div className="sm:h-10 md:h-16 overflow-hidden">
          {goal.description ? (
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed sm:line-clamp-2 md:line-clamp-3">
              {goal.description}
            </p>
          ) : (
            <div></div>
        ...
        {/* Grid Row 4: Spacer - Absorbs remaining space */}
        <div className="min-h-0"></div>
        
        {/* Grid Row 5: Progress Section - FIXED HEIGHT (ALIGNMENT TARGET) */}
        <div className="h-16">
        ...
        {/* Grid Row 6: Footer & Milestone Details - Flexible height */}
        <div className="space-y-3">
          ...
          {/* Footer - Target date and time status */}
          <div className="flex items-center justify-between text-xs min-h-[1rem] pt-2">
