-- Simple Referral System Setup - Works with existing user_plans table
-- Run this if you get column errors with the minimal setup
...
-- 3. Add referral columns to existing user_plans table
DO $$ 
BEGIN
...
-- 5. Create user referral dashboard view (simple version)
...
SELECT 
    ...
    COALESCE(up.plan_type, 'Free') as plan_status,
    up.subscription_end_date,
    COALESCE(up.referral_extension_days, 0) as referral_extension_days
...
-- 7. Simplified referral benefits function (works with any user_plans structure)
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' THEN
        ...
        IF FOUND THEN
            -- Check if plan changed to premium (any paid plan)
            IF (NEW.plan_type = 'premium' OR NEW.plan_type = 'Premium') 
               AND (OLD.plan_type IS NULL OR OLD.plan_type = 'free' OR OLD.plan_type = 'Free') THEN
                -- Activate referral
                UPDATE referrals 
            ...
            ELSIF (NEW.plan_type = 'free' OR NEW.plan_type = 'Free' OR NEW.plan_type IS NULL)
                  AND (OLD.plan_type = 'premium' OR OLD.plan_type = 'Premium') THEN
                -- Deactivate referral
                UPDATE referrals 
            ...
            -- Update referral extension days if column exists
            UPDATE user_plans 
            ...
            AND EXISTS (
                ...
                WHERE table_name = 'user_plans' 
                AND column_name = 'referral_extension_days'
            );
        END IF;
    END IF;
